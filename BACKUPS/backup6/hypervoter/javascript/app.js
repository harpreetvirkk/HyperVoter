    ...
    if (username && password) {
        ...
        for (let i = 0; i<voters_list.length; i++){
            if (voters_list[i].voterId == username){
                ...
                if (voters_list[i].pin != password){
                    // console.log("Incorrect PIN");
                    response.render(__dirname + "/public/voter-dashboard/voter-registration.html", {_:"Incorrect PIN"});
                    break;
                } else {
                    console.log("Identity Verified!\n");
                    request.session.loggedin = true;
        ...
        if (flag != 1){
            console.log("You have not been pre-approved by EC to register as a voter!");
            response.render(__dirname + "/public/voter-dashboard/voter-registration.html", {_:"You have not been pre-approved by EC to register as a voter!"});
        }
    ...
    async function queryVote(){
        try{
            ...
            if (query_results.length == 0){
                let que_res_string = "Vote object has not yet been casted!";
            } else {
                let que_res_string = "Vote ID: " + vid + "   , Candidate ID: "+ query_results.ownerId +"<br></br>";;
            }
